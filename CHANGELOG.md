...
- Queue rows use click/shift-click only for selection. Selecting one task reveals an edit icon on the queue toolbar; selecting multiple tasks replaces it with the batch controls. The previous full-record row hover is removed. Name, command, status, priority, automatic/candidate GPU placement, per-GPU args, estimated DRAM, and the HBM gate can be updated in place; running tasks must be paused first and cannot be assigned manually without a real process.
